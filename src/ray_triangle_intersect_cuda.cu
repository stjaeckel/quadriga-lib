...
__device__ __forceinline__
    uint64_t
    pack_hit(float t, uint32_t face_idx)
{
...
__device__ __forceinline__ float unpack_t(uint64_t packed)
{
...
__device__ __forceinline__ void enqueue_warp(bool has_hit, uint32_t ray_idx, uint16_t aabb_idx,
...
// ============================================================================
// Kernel 0: init_per_ray_state (Section 4.0)
// ============================================================================

__global__ void init_per_ray_state(
...
// ============================================================================
// Kernel 1: aabb_test_and_enqueue (Section 4.1)
...
// ============================================================================
// Kernel 2: moller_trumbore_fbs (Section 4.2)
        ...
        uint8_t *__restrict__ wq_had_hit)
{
    ...
    {
        uint64_t packed = pack_hit(local_best_t, local_best_face);
...
// ============================================================================
// Kernel 3: moller_trumbore_sbs (Section 4.3)
        ...
        unsigned long long *__restrict__ sbs_packed)
{
    ...
    uint32_t fbs_face = unpack_face((uint64_t)fbs_packed[ray_idx]);
...
// ============================================================================
// Kernel 4: finalize_outputs (Section 4.4)
// Unpacks packed FBS/SBS into user-facing Wf, Ws, If, Is, hit_cnt.
// ============================================================================

__global__ void finalize_outputs(
    const unsigned long long *__restrict__ fbs_packed,
    ...
    bool count_hits)
{
    ...
    float Wf_val = unpack_t(fbs);
    float Ws_val = unpack_t(sbs);
    uint32_t If_val = unpack_face(fbs);
    uint32_t Is_val = unpack_face(sbs);
...
#ifdef QD_RTI_DEBUG
    // Debug invariant: SBS must never be closer than FBS
    assert(Ws_val >= Wf_val);
#endif
}
...
static inline int ceil_log2(uint32_t n)
{
    if (n <= 1u)
        return 1;
    int bits = 0;
...
static inline void upload_float(float *&d_ptr, const float *h_ptr, size_t count)
{
...
static inline void stage_ray_input(float *pinned, size_t batch_size,
...
static inline void unstage_ray_output(const char *pinned, size_t batch_size,
                                      float *Wf, float *Ws,
...
// ============================================================================
// qd_RTI_CUDA — main entry point (Section 8, 5.1, 5.2)
// ============================================================================
    ...
    int gpu_id)
{
    ...
    upload_float(buf.d_Tx, Tx, n_mesh);
    upload_float(buf.d_Ty, Ty, n_mesh);
    upload_float(buf.d_Tz, Tz, n_mesh);
    upload_float(buf.d_E1x, E1x, n_mesh);
    upload_float(buf.d_E1y, E1y, n_mesh);
    upload_float(buf.d_E1z, E1z, n_mesh);
    upload_float(buf.d_E2x, E2x, n_mesh);
    upload_float(buf.d_E2y, E2y, n_mesh);
    upload_float(buf.d_E2z, E2z, n_mesh);
    ...
    // AABB bounds
    upload_float(buf.d_Xmin, Xmin, n_sub);
    upload_float(buf.d_Xmax, Xmax, n_sub);
    upload_float(buf.d_Ymin, Ymin, n_sub);
    upload_float(buf.d_Ymax, Ymax, n_sub);
    upload_float(buf.d_Zmin, Zmin, n_sub);
    upload_float(buf.d_Zmax, Zmax, n_sub);
    ...
    int num_aabb_bits = ceil_log2((uint32_t)n_sub);
    if (num_aabb_bits > 16)
        num_aabb_bits = 16;
    ...
    // Stage batch 0 into pinned memory BEFORE the loop
    {
        ...
        stage_ray_input(buf.h_ray_input_pinned[0], batch_size,
    ...
    for (size_t batch_idx = 0; batch_idx < n_batches; ++batch_idx)
    {
        ...
        if (batch_idx > 0)
        {
            ...
            unstage_ray_output(buf.h_ray_output_pinned[sp], batch_size,
        ...
        init_per_ray_state<<<grid0, BLOCK_SIZE, 0, cs>>>(
        ...
        aabb_test_and_enqueue<<<grid0, BLOCK_SIZE, 0, cs>>>(
        ...
        if (qt > 0)
        {
            ...
            moller_trumbore_fbs<<<grid2, BLOCK_SIZE, 0, cs>>>(
        ...
        if (batch_idx + 1 < n_batches)
        {
            ...
            stage_ray_input(buf.h_ray_input_pinned[next_s], batch_size,
        ...
        if (nc > 0)
        {
            ...
            moller_trumbore_sbs<<<grid3, BLOCK_SIZE, 0, cs>>>(
        ...
        finalize_outputs<<<grid0, BLOCK_SIZE, 0, cs>>>(
            buf.d_fbs_packed[s],
            buf.d_sbs_packed[s],
            buf.d_hit_cnt_atomic[s],
    ...
    // --- Drain last batch ---
    {
        ...
        unstage_ray_output(buf.h_ray_output_pinned[last_s], batch_size,
                           Wf, Ws, If, Is, hit_cnt, count_hits,
