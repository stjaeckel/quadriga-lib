...
template <typename dtype>
arma::uvec quadriga_lib::obj_overlap_test(const arma::Mat<dtype> *mesh, const arma::uvec *obj_ind,
                                          std::vector<std::string> *reason, dtype tolerance)
{
    ...
    if (obj_ind == nullptr)
        throw std::invalid_argument("Input 'obj_ind' cannot be NULL.");
    ...
    arma::uvec obj_ids = arma::unique(*obj_ind);      // Unique object IDs
    arma::uword n_obj = obj_ids.n_elem;               // Number of objects
    arma::uword *p_obj_ids = obj_ids.memptr();        // Unique object IDs (pointer)
    const arma::uword *p_obj_ind = obj_ind->memptr(); // Object IDs (pointer)
    ...
    arma::Mat<dtype> aabb(6, n_obj, arma::fill::none);
    dtype *box = aabb.memptr(); // Object bounding box
    {
        // Initialize bounding box coordinates
        for (arma::uword i = 0; i < aabb.n_elem; ++i)
            box[i] = (i % 2) ? -INFINITY : INFINITY;

        // Get the initial bounding box for the first object
        arma::uword i_obj = 0, i_obj_prev = 0;
        dtype Xmin = box[0], Xmax = box[1],
        ...
        for (arma::uword i_mesh = 0; i_mesh < n_mesh; ++i_mesh)
        {
            ...
            if (i_obj != i_obj_prev)
            {
                box[6 * i_obj_prev] = Xmin, Xmin = box[6 * i_obj];
                box[6 * i_obj_prev + 1] = Xmax, Xmax = box[6 * i_obj + 1];
                box[6 * i_obj_prev + 2] = Ymin, Ymin = box[6 * i_obj + 2];
                box[6 * i_obj_prev + 3] = Ymax, Ymax = box[6 * i_obj + 3];
                box[6 * i_obj_prev + 4] = Zmin, Zmin = box[6 * i_obj + 4];
                box[6 * i_obj_prev + 5] = Zmax, Zmax = box[6 * i_obj + 5];
            }
            i_obj_prev = i_obj;

            dtype x = p_mesh[i_mesh],
                  y = p_mesh[i_mesh + n_mesh],
                  z = p_mesh[i_mesh + 2 * n_mesh];

            Xmin = (x < Xmin) ? x : Xmin, Xmax = (x > Xmax) ? x : Xmax;
            Ymin = (y < Ymin) ? y : Ymin, Ymax = (y > Ymax) ? y : Ymax;
            Zmin = (z < Zmin) ? z : Zmin, Zmax = (z > Zmax) ? z : Zmax;

            x = p_mesh[i_mesh + 3 * n_mesh],
            y = p_mesh[i_mesh + 4 * n_mesh],
            z = p_mesh[i_mesh + 5 * n_mesh];

            Xmin = (x < Xmin) ? x : Xmin, Xmax = (x > Xmax) ? x : Xmax;
            Ymin = (y < Ymin) ? y : Ymin, Ymax = (y > Ymax) ? y : Ymax;
            Zmin = (z < Zmin) ? z : Zmin, Zmax = (z > Zmax) ? z : Zmax;

            x = p_mesh[i_mesh + 6 * n_mesh],
            y = p_mesh[i_mesh + 7 * n_mesh],
            z = p_mesh[i_mesh + 8 * n_mesh];
        ...
        box[6 * i_obj_prev] = Xmin;
        box[6 * i_obj_prev + 1] = Xmax;
        box[6 * i_obj_prev + 2] = Ymin;
        box[6 * i_obj_prev + 3] = Ymax;
        box[6 * i_obj_prev + 4] = Zmin;
        box[6 * i_obj_prev + 5] = Zmax;
    }
...
#pragma omp parallel for
    for (int i_obj_int = 0; i_obj_int < (int)n_obj; ++i_obj_int)
    {
        ...
        arma::uword n_edges = 3 * n_faces;
        obj_faces[i_obj] = arma::Mat<dtype>(18, n_faces, arma::fill::none);
        obj_edges[i_obj] = arma::Mat<dtype>(6, n_edges, arma::fill::none);

        dtype *p_faces = obj_faces[i_obj].memptr();
        dtype *p_edges = obj_edges[i_obj].memptr();

        arma::uword i_face = 0;
        arma::uword obj_id = p_obj_ids[i_obj];

        for (arma::uword i_mesh = 0; i_mesh < n_mesh; ++i_mesh)
            if (p_obj_ind[i_mesh] == obj_id)
            {
                arma::uword o_face = 18 * i_face;

                // Vertex 1
                dtype x0 = p_mesh[i_mesh];
                dtype y0 = p_mesh[i_mesh + n_mesh];
                dtype z0 = p_mesh[i_mesh + 2 * n_mesh];
                ...
                p_edges[6 * i_face] = x0;
                p_edges[6 * i_face + 1] = y0;
                p_edges[6 * i_face + 2] = z0;

                p_edges[6 * (i_face + 2 * n_faces) + 3] = x0;
                p_edges[6 * (i_face + 2 * n_faces) + 4] = y0;
                p_edges[6 * (i_face + 2 * n_faces) + 5] = z0;

                // Vertex 2
                dtype x1 = p_mesh[i_mesh + 3 * n_mesh];
                dtype y1 = p_mesh[i_mesh + 4 * n_mesh];
                dtype z1 = p_mesh[i_mesh + 5 * n_mesh];

                p_faces[o_face + 3] = x1;
                p_faces[o_face + 4] = y1;
                p_faces[o_face + 5] = z1;

                p_edges[6 * (i_face + n_faces)] = x1;
                p_edges[6 * (i_face + n_faces) + 1] = y1;
                p_edges[6 * (i_face + n_faces) + 2] = z1;

                p_edges[6 * i_face + 3] = x1;
                p_edges[6 * i_face + 4] = y1;
                p_edges[6 * i_face + 5] = z1;

                // Vertex 3
                dtype x2 = p_mesh[i_mesh + 6 * n_mesh];
                dtype y2 = p_mesh[i_mesh + 7 * n_mesh];
                dtype z2 = p_mesh[i_mesh + 8 * n_mesh];

                p_faces[o_face + 6] = x2;
                p_faces[o_face + 7] = y2;
                p_faces[o_face + 8] = z2;

                p_edges[6 * (i_face + 2 * n_faces)] = x2;
                p_edges[6 * (i_face + 2 * n_faces) + 1] = y2;
                p_edges[6 * (i_face + 2 * n_faces) + 2] = z2;

                p_edges[6 * (i_face + n_faces) + 3] = x2;
                p_edges[6 * (i_face + n_faces) + 4] = y2;
                p_edges[6 * (i_face + n_faces) + 5] = z2;
                ...
                p_faces[o_face + 9] = x2;
                p_faces[o_face + 10] = y2;
                p_faces[o_face + 11] = z2;

                // P0 -> P1 (X1) as a direction for U
                dtype len = (dtype)1.0 / std::sqrt(x1 * x1 + y1 * y1 + z1 * z1);
                x1 *= len, y1 *= len, z1 *= len;
                p_faces[o_face + 12] = x1;
                p_faces[o_face + 13] = y1;
                p_faces[o_face + 14] = z1;

                // V = N x U, store result in X2
                crossp(x2, y2, z2, x1, y1, z1, &x2, &y2, &z2, true);
                p_faces[o_face + 15] = x2;
                p_faces[o_face + 16] = y2;
                p_faces[o_face + 17] = z2;
...
#pragma omp parallel for
    for (int i_obj_1_int = 0; i_obj_1_int < (int)n_obj; ++i_obj_1_int)
    {
        ...
        dtype ax_low = box[6 * i_obj_1] - eps, ax_high = box[6 * i_obj_1 + 1] + eps,
              ay_low = box[6 * i_obj_1 + 2] - eps, ay_high = box[6 * i_obj_1 + 3] + eps,
              az_low = box[6 * i_obj_1 + 4] - eps, az_high = box[6 * i_obj_1 + 5] + eps;

        for (arma::uword i_obj_2 = 0; i_obj_2 < n_obj; ++i_obj_2)
        {
            if (i_obj_1 == i_obj_2)
                continue;
            ...
            dtype bx_low = box[6 * i_obj_2] - eps, bx_high = box[6 * i_obj_2 + 1] + eps,
                  by_low = box[6 * i_obj_2 + 2] - eps, by_high = box[6 * i_obj_2 + 3] + eps,
                  bz_low = box[6 * i_obj_2 + 4] - eps, bz_high = box[6 * i_obj_2 + 5] + eps;
            ...
            if (test_condition)
                for (arma::uword i_face = 0; i_face < n_faces_1; ++i_face)
                {
                    arma::uword offset = i_face * 18;
                    for (arma::uword i_val = 0; i_val < 9; ++i_val)
...
#pragma omp critical
            ...
            // Test if edges of OBJ-2 intersect with faces of OBJ-1
            std::string findings;
            bool nothing_to_report_so_far = true;

            // Get pointers to data
            arma::uword n_faces = n_faces_1;
            arma::uword n_edges = 3 * n_faces_2;
            ...
            for (arma::uword i_edge = 0; i_edge < n_edges; ++i_edge)
                if (p_hit[i_edge])
                    ++p_hit[i_edge];
            ...
            for (arma::uword i_face = 0; i_face < n_faces; ++i_face)
            {
                ...
                dtype x1 = p_faces[o_face + 3],
                      y1 = p_faces[o_face + 4],
                      z1 = p_faces[o_face + 5];

                dtype x2 = p_faces[o_face + 6],
                      y2 = p_faces[o_face + 7],
                      z2 = p_faces[o_face + 8];

                // Face normal vectors
                dtype Nx = p_faces[o_face + 9],
                      Ny = p_faces[o_face + 10],
                      Nz = p_faces[o_face + 11];

                dtype Ux = p_faces[o_face + 12],
                      Uy = p_faces[o_face + 13],
                      Uz = p_faces[o_face + 14];

                dtype Vx = p_faces[o_face + 15],
                      Vy = p_faces[o_face + 16],
                      Vz = p_faces[o_face + 17];
                ...
                for (arma::uword i_edge = 0; i_edge < n_edges; ++i_edge)
                {
                    ...
                    if (dist_o < tolerance && dist_d < tolerance)
                    {
                        ...
                        arma::uword o_edge_face = 18 * (i_edge % (n_edges / 3));
                        dtype Mx = p_edge_faces[o_edge_face + 9],
                              My = p_edge_faces[o_edge_face + 10],
                              Mz = p_edge_faces[o_edge_face + 11];
...
#pragma omp critical
    ...
    arma::uvec obj_ids_intersecting(n_intersecting);
    n_intersecting = 0;
    for (arma::uword i_obj = 0; i_obj < n_obj; ++i_obj)
        if (intersecting_objects[i_obj])
        {
...
template arma::uvec quadriga_lib::obj_overlap_test(const arma::Mat<float> *mesh, const arma::uvec *obj_ind, std::vector<std::string> *reason, float tolerance);
template arma::uvec quadriga_lib::obj_overlap_test(const arma::Mat<double> *mesh, const arma::uvec *obj_ind, std::vector<std::string> *reason, double tolerance);
