...

{
    ...
    const int n_point_i = (int)n_point; // Number of triangles as int
...
#pragma omp parallel for
    for (int i_ray = 0; i_ray < n_ray_i; ++i_ray) // Ray loop
    {
        ...
        for (size_t i_sub = 0; i_sub < n_sub; ++i_sub)
        {
            ...
            float b0_low = Xmin[i_sub], b0_high = Xmax[i_sub],
                  b1_low = Ymin[i_sub], b1_high = Ymax[i_sub],
                  b2_low = Zmin[i_sub], b2_high = Zmax[i_sub];
            ...
            // Calculate the vertex box at advanced wavefront
            for (int i = 0; i < 8; ++i)
            {
