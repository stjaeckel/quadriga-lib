...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nlhs > 4)
        mexErrMsgIdAndTxt("quadriga_lib:arrayant_combine_pattern:IO_error", "Can have at most 4 outputs.");
    ...
    if (use_single)
    {
        quadriga_lib::arrayant<float> arrayant_in;
        arrayant_in.e_theta_re = qd_mex_reinterpret_Cube<float>(prhs[0]);
        arrayant_in.e_theta_im = qd_mex_reinterpret_Cube<float>(prhs[1]);
        arrayant_in.e_phi_re = qd_mex_reinterpret_Cube<float>(prhs[2]);
        arrayant_in.e_phi_im = qd_mex_reinterpret_Cube<float>(prhs[3]);
        arrayant_in.azimuth_grid = qd_mex_reinterpret_Col<float>(prhs[4]);
        arrayant_in.elevation_grid = qd_mex_reinterpret_Col<float>(prhs[5]);
        if (nrhs > 6)
            arrayant_in.element_pos = qd_mex_reinterpret_Mat<float>(prhs[6]);
        if (nrhs > 7)
            arrayant_in.coupling_re = qd_mex_reinterpret_Mat<float>(prhs[7]);
        if (nrhs > 8)
            arrayant_in.coupling_im = qd_mex_reinterpret_Mat<float>(prhs[8]);
        if (nrhs > 9)
            arrayant_in.center_frequency = qd_mex_get_scalar<float>(prhs[9], "center_frequency");
        arrayant_in.read_only = true;

        std::string error_message = arrayant_in.is_valid();
        if (!error_message.empty())
            mexErrMsgIdAndTxt("quadriga_lib:arrayant_combine_pattern:IO_error", error_message.c_str());

        quadriga_lib::arrayant<float> arrayant_out;
        CALL_QD(arrayant_out = arrayant_in.combine_pattern());
        if (nlhs > 0)
    ...
    {
        quadriga_lib::arrayant<double> arrayant_in;

        arrayant_in.e_theta_re = qd_mex_reinterpret_Cube<double>(prhs[0]);
        arrayant_in.e_theta_im = qd_mex_reinterpret_Cube<double>(prhs[1]);
        arrayant_in.e_phi_re = qd_mex_reinterpret_Cube<double>(prhs[2]);
        arrayant_in.e_phi_im = qd_mex_reinterpret_Cube<double>(prhs[3]);
        arrayant_in.azimuth_grid = qd_mex_reinterpret_Col<double>(prhs[4]);
        arrayant_in.elevation_grid = qd_mex_reinterpret_Col<double>(prhs[5]);
        if (nrhs > 6)
            arrayant_in.element_pos = qd_mex_reinterpret_Mat<double>(prhs[6]);
        if (nrhs > 7)
            arrayant_in.coupling_re = qd_mex_reinterpret_Mat<double>(prhs[7]);
        if (nrhs > 8)
            arrayant_in.coupling_im = qd_mex_reinterpret_Mat<double>(prhs[8]);
        if (nrhs > 9)
            arrayant_in.center_frequency = qd_mex_get_scalar<double>(prhs[9], "center_frequency");
        arrayant_in.read_only = true;
        ...
        quadriga_lib::arrayant<double> arrayant_out;
        CALL_QD(arrayant_out = arrayant_in.combine_pattern());
        if (nlhs > 0)
            plhs[0] = qd_mex_copy2matlab(&arrayant_out.e_theta_re);
        if (nlhs > 1)
