...
- Outputs are always single precision `single`
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxIsSingle(prhs[0]))
        X = qd_mex_reinterpret_Cube<float>(prhs[0]); // No copy
    else
        X = qd_mex_typecast_Cube<float>(prhs[0]);
    ...
    arma::fcube P, Q;
    if (nlhs > 0)
        plhs[0] = qd_mex_init_output(&P, X.n_rows, X.n_cols, X.n_slices);

    if (nlhs > 1 && !cos_only)
        plhs[1] = qd_mex_init_output(&Q, X.n_rows, X.n_cols, X.n_slices);
    ...
    // Serialize cubes to vectors (map only memory)
    arma::fvec x = arma::fvec(X.memptr(), X.n_elem, false, true);
    arma::fvec p = arma::fvec(P.memptr(), P.n_elem, false, true);

    arma::fvec q;
    if (Q.n_elem != 0)
        q = arma::fvec(Q.memptr(), Q.n_elem, false, true);

    // Call Quadriga-Lib C++ API
    if (cos_only)
        CALL_QD(quadriga_lib::fast_sincos(x, nullptr, &p));
    else if (nlhs > 1)
        CALL_QD(quadriga_lib::fast_sincos(x, &p, &q));
    else
        CALL_QD(quadriga_lib::fast_sincos(x, &p));
}