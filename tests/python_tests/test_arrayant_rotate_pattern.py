...
class test_version(unittest.TestCase):
    ...
    def test(self):
        ...
        ant = arrayant.generate('custom', az_3dB=5, el_3dB=20 )
        out = arrayant.rotate_pattern(ant, 0, 0, 90)
        ...
        out = arrayant.rotate_pattern(ant, 0, -45, 0)
        ...
        out = arrayant.rotate_pattern(ant, 45, 0, 0)
        ...
        out = arrayant.rotate_pattern(ant, 180, 180, 180)
        npt.assert_almost_equal(out["e_theta_re"], ant["e_theta_re"], decimal=13)

        out = arrayant.rotate_pattern(ant, 90, 0, 0, 1)
        npt.assert_almost_equal(out["e_theta_re"][90,180,0], v, decimal=14)

        # Single element rotation
        with self.assertRaises(ValueError) as context:
            arrayant.rotate_pattern(ant, 0, 0, 90, 1, 3)
        self.assertEqual(str(context.exception), "Input parameter 'element' out of bound.")

        ant = arrayant.copy_element(ant,0,[1,2])
        ant = arrayant.copy_element(ant,[0,1],[1,2])
        out = arrayant.rotate_pattern(ant, 0, 0, 90, element=[1,2])
        ...
        with self.assertRaises(TypeError) as context:
            arrayant.rotate_pattern(ant["e_theta_re"])
