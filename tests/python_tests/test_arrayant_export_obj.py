...
COLORMAPS = ("jet", "parula", "winter", "hot", "turbo", "copper",
             "spring", "cool", "gray", "autumn", "summer")


def _copy_ant(ant):
    """Deep copy of an arrayant dict (numpy fields copied, scalars/strings kept)."""
    return {k: (np.array(v) if isinstance(v, np.ndarray) else v) for k, v in ant.items()}
...
def _stack_two(ant_a, ant_b):
    ...
    mf = {}
    for key in ("e_theta_re", "e_theta_im", "e_phi_re", "e_phi_im"):
        mf[key] = np.stack([np.asarray(ant_a[key], dtype=float),
                            np.asarray(ant_b[key], dtype=float)], axis=3)
    for key in ("azimuth_grid", "elevation_grid", "element_pos", "coupling_re", "coupling_im"):
        if key in ant_a:
...
class test_case(unittest.TestCase):
    ...
    def _read(self, obj_path):
        return quadriga_lib.RTtools.obj_file_read(obj_path)
    ...
    def test_basic_file_creation(self):
        ...
        self.assertEqual(mtl_prop.shape, (n_tri, 9))
    ...
    def test_object_radius_scaling(self):
        ...
        v1 = self._read(self._export(ant, object_radius=1.0, icosphere_n_div=2))[2]
        v3 = self._read(self._export(ant, object_radius=3.0, icosphere_n_div=2))[2]
    ...
    def test_element_selection(self):
        ...
        obj_all = self._read(self._export(ant, object_radius=1.0, icosphere_n_div=2))[4]
        # element = [0] -> export only the first element (0-based index).
        obj_one = self._read(self._export(ant, object_radius=1.0, icosphere_n_div=2,
                                          element=[0]))[4]
    ...
    def test_directivity_range(self):
        ...
        for drange in (10.0, 30.0, 50.0):
            p = self._export(ant, directivity_range=drange, object_radius=1.0, icosphere_n_div=2)
            self.assertTrue(os.path.exists(p))
    ...
    def test_multifreq(self):
        ...
        v0 = self._read(self._export(mf, freq_ind=0, **kw))[2]
        v1 = self._read(self._export(mf, freq_ind=1, **kw))[2]
        ...
        v_a = self._read(self._export(base, **kw))[2]
        v_b = self._read(self._export(ent_b, **kw))[2]
        npt.assert_allclose(v0, v_a, atol=1e-9, rtol=0)
        npt.assert_allclose(v1, v_b, atol=1e-9, rtol=0)

        # Omitted freq_ind defaults to 0.
        v_default = self._read(self._export(mf, **kw))[2]
    ...
    def test_errors(self):
        ...
        with self.assertRaises(ANY_ERR):
            quadriga_lib.arrayant.export_obj_file(os.path.join(self.tmpdir, "ant.txt"), base)
