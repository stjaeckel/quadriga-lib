...
- **`remove_delay_phase = true` (default):** removes the baked-in phase `exp(-j * 2 * pi * freq_in[f] * delay)` before SLERP, then re-applies the full delay phase at each output frequency; required when input comes from [get_channels_multifreq](#get_channels_multifreq) or [get_channels_spherical](#get_channels_spherical), which bake the delay phase into coefficients at each input frequency; set to `false` only when input coefficients are pure slowly-varying envelopes without baked-in delay phase
...
- **`remove_delay_phase`** *(optional)* — Remove baked-in delay phase before interpolation and re-apply analytically; must be `true` for output from [get_channels_multifreq](#get_channels_multifreq) or [get_channels_spherical](#get_channels_spherical)
